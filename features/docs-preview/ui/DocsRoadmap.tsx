      ...
      <ol className="flex flex-wrap items-center justify-start gap-2 text-sm text-slate-500">
        {items.map((item, index) => {
          const content = item.isHome ? (
            <span className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-700 transition hover:border-slate-300 hover:bg-slate-100">
              <HomeIcon />
            </span>
          ) : (
            <span
